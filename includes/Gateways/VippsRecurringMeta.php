...
class MinSponsor_VippsRecurringMeta {
    ...
    public function init() {
        ...
        add_filter('vipps_recurring_agreement_data', array($this, 'add_agreement_metadata'), 10, 2);
        add_filter('vipps_recurring_charge_data', array($this, 'add_charge_metadata'), 10, 3);
        
        // Alternative hook names
        add_filter('woo_vipps_recurring_agreement_args', array($this, 'add_agreement_metadata'), 10, 2);
        add_filter('woo_vipps_recurring_charge_args', array($this, 'add_charge_metadata'), 10, 3);
        
        // MobilePay specific hooks (if different plugin is used)
        add_filter('mobilepay_recurring_agreement_data', array($this, 'add_agreement_metadata'), 10, 2);
        add_filter('mobilepay_recurring_charge_data', array($this, 'add_charge_metadata'), 10, 3);
        
        // Generic hooks that might be used by different implementations
        add_action('woocommerce_subscription_payment_method_updated', array($this, 'update_agreement_metadata'), 10, 2);
    }
     ...
     */
    public function add_agreement_metadata($agreement_data, $order) {
        $minsponsor_metadata = MinSponsor_MetaFlow::get_payment_metadata($order);
        
        if (!empty($minsponsor_metadata)) {
            // Add metadata to agreement
            if (!isset($agreement_data['metadata'])) {
                $agreement_data['metadata'] = array();
            }
            ...
            if (!isset($agreement_data['userDefined'])) {
                $agreement_data['userDefined'] = array();
            }
            ...
            error_log(sprintf(
                ...
                $order->get_id(),
                json_encode($minsponsor_metadata)
            ));
        }
     ...
     */
    public function add_charge_metadata($charge_data, $subscription, $order) {
     ...
     */
    public function update_agreement_metadata($subscription, $new_payment_method) {
        // Only process if new payment method is Vipps/MobilePay
        if (!in_array($new_payment_method, array('vipps_recurring', 'mobilepay_recurring'))) {
            return;
        }
     ...
     */
    private function get_subscription_metadata($subscription) {
        $metadata = array();
     ...
     */
    private function build_sponsorship_description($metadata) {
        $parts = array();
     ...
     * @return bool
     */
    public static function is_vipps_recurring_available() {
     ...
     * @return bool
     */
    public static function is_mobilepay_recurring_available() {
     ...
     */
    private function get_vipps_agreement_id($subscription) {
     ...
     */
    public function manual_update_agreement($subscription, $metadata) {
